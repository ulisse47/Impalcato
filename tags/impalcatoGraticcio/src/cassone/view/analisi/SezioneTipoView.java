	...
	public static JPanel getInstance() {
		if (container == null) {
			...
			container.setBorder(new TitledBorder(new EtchedBorder(
					EtchedBorder.RAISED, Color.white, Color.gray), "SEZIONE"));
...
/*		cmbConcio = new JComboBox();
		cmbConcio.addActionListener(acs);
		cmbConcio.setActionCommand("SET_CONCIO");
		cmbConcio.setVisible(false);
		cmbConcio.setBackground(Color.red);
		cmbConcio.setForeground(Color.BLUE);
		add(cmbConcio);

		cmbSoletta = new JComboBox();
		cmbSoletta.addActionListener(acs);
		cmbSoletta.setActionCommand("SET_CONCIO");
		cmbSoletta.setVisible(false);
		cmbSoletta.setBackground(Color.red);
		cmbSoletta.setForeground(Color.BLUE);
		add(cmbSoletta);
*/
	...
	class AscButton implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (comboSource.getActionCommand() == "SET_CONCIO") {
				SezioneMetallica concio = (SezioneMetallica) comboSource
						.getSelectedItem();
				Progetto prg = Progetto.getInstance();
				try {
					prg.setConcio(concio, true);

				} catch (Exception ex) {
				}
				comboSource.setVisible(false);
