...
public class DlgModificaNomiConci extends JDialog {
  private TabellaNomi jTable1;
  private JButton jButtonOk = new JButton();
  private JButton jButtonCancel = new JButton();
  ArrayList<SezioneMetallica> conci;
  //  private JButton jButtonAdd = new JButton();
  
//  private Struttura str;

  public DlgModificaNomiConci(ArrayList<SezioneMetallica> conci) throws HeadlessException {

	this.conci=conci;
	this.setModal(true);
    jTable1 =  new TabellaNomi();
    try {
      jbInit();
    }
    catch(Exception e) {
...
*/
 ...
 //   this.getContentPane().add(jButtonAdd, null);
  ...
  private class TabellaNomi extends JTable{
    private Object[][] data;
    private int nC;
    ...
    public TabellaNomi() {
      ...
      final String[] names = {"CONCIO", "NOME"};
      nC = conci.size();
      data = new Object[nC][2];
      for(int i = 0; i < nC; i++){
        data[i][0] = new Integer(i+1);
       data[i][1] = new String(conci.get(i).getName());
      }
      ...
      TableModel dataModel = new AbstractTableModel() {
        ...
        public void aggiornaTabella(){
          	Progetto prg = Progetto.getInstance();
        	final String[] names = {"CONCIO", "NOME"};
            ArrayList<SezioneMetallica> conci = prg.getConci();
            nC = conci.size();
            data = new Object[nC][2];
            for(int i = 0; i < nC; i++){
              data[i][0] = new Integer(i+1);
...
//	  private Object[][] data = new Object[sez.size()][2];
...
//    	Progetto prg = Progetto.getInstance();
 //   	Campate c = prg.getCampate();
      for(int i =0; i < jTable1.nC; i++){
        String nome = (String)jTable1.data[i][1];
        conci.get(i).setName(nome);
      }
      this.dispose();
      }catch(Exception ex){
        JOptionPane.showMessageDialog(null, "non è stato possibile modificare i nomi",
                "Warning", JOptionPane.ERROR_MESSAGE);
      }
  }
